...
import { TestToken } from '../constants/addresses';
...
function displayResult(
    ...
    if (!amtInSor.eq(amtInQuery) || !amtOutSor.eq(amtOutQuery)) {
        ...
        console.log('\nSOR and Query Match');
        console.log(
            `${swapTypeStr} ${tokenIn.symbol}>${tokenOut.symbol
...
async function testSwaps() {
    ...
    const swaps = [
        {
            tokenIn: ADDRESSES[networkId].DAI_from_AAVE,
            tokenOut: ADDRESSES[networkId].USDC_from_AAVE,
            swapType: SwapTypes.SwapExactIn,
            amt: parseFixed('100', 18),
        },
        {
            tokenIn: ADDRESSES[networkId].DAI_from_AAVE,
            tokenOut: ADDRESSES[networkId].USDC_from_AAVE,
        ...
        {
            tokenIn: ADDRESSES[networkId].USDT_from_AAVE,
            tokenOut: ADDRESSES[networkId].USDC_from_AAVE,
            swapType: SwapTypes.SwapExactIn,
            amt: parseFixed('1000', 6),
        },
        {
            tokenIn: ADDRESSES[networkId].USDT_from_AAVE,
            tokenOut: ADDRESSES[networkId].USDC_from_AAVE,
    ...
    for (let i = 0; i < swaps.length; i++) {
        const { tokenIn, tokenOut, swapType, amt } = swaps[i];
        const { swapInfo, cost, deltas } = results[i];
        displayResult(tokenIn, tokenOut, swapType, amt, swapInfo, cost, deltas);
    }
}

// ts-node ./balancer/sorBoostedPools.ts
testSwaps();
