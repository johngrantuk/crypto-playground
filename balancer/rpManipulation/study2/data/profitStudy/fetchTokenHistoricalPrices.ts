...
export async function fetchHistoricalPricesMap(params: {
    config: ProfitStudyConfig;
    uniqueAddresses: string[];
}): Promise<Map<string, PricePoint[]>> {
    const { config, uniqueAddresses } = params;
    const map = new Map<string, PricePoint[]>();
    const keys = [...new Set(uniqueAddresses.map(tokenKey))].filter(Boolean);

    for (const batch of chunk(keys, config.historicalPricesBatchSize)) {
        if (batch.length === 0) {
            continue;
        }
        const data = await graphQlRequestWithDelay<HistResponse>({
            config,
            query: HIST_PRICES_QUERY,
            variables: {
                addresses: batch,
