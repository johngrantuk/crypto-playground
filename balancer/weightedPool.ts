...
async function createPool(weightedPoolFactoryAddr: string, poolInfo: WeightedPoolInfo, provider = getProvider(), wallet = getWallet(provider)): Promise<string> {
    const poolFactory = new Contract(weightedPoolFactoryAddr, weightedPoolFactoryAbi, provider);
    poolFactory.connect(wallet);
    console.log('Creating pool...');
    const address = await poolFactory.callStatic.create(poolInfo.name,
        poolInfo.symbol,
        poolInfo.tokens,
        poolInfo.weights,
    ...
    const tx = await poolFactory.connect(wallet).create(
        poolInfo.name,
        poolInfo.symbol,
        poolInfo.tokens,
        poolInfo.weights,
...
export function encodeJoinWeightedPool(
    ...
    } else if (joinData.kind === JoinTypes.JOIN_WEIGHTED_POOL_EXACT_TOKENS_IN_FOR_BPT_OUT_TAG) {
        return defaultAbiCoder.encode(
            ['uint256', 'uint256[]', 'uint256'],
            [JoinTypes.JOIN_WEIGHTED_POOL_EXACT_TOKENS_IN_FOR_BPT_OUT_TAG, joinData.amountsIn, joinData.minimumBPT]
        );
    } else {
        return defaultAbiCoder.encode(
            ['uint256', 'uint256', 'uint256'],
            [JoinTypes.JOIN_WEIGHTED_POOL_TOKEN_IN_FOR_EXACT_BPT_OUT_TAG, joinData.bptAmountOut, joinData.enterTokenIndex]
        );
    }
}


async function joinPool(joinData: JoinWeightedPoolData, poolId: string, assets: string[], maxAmountsIn: string[], provider = getProvider(), wallet = getWallet(provider)) {
    ...
    let tx = await vaultContract.connect(wallet).joinPool(poolId, wallet.address, wallet.address, {
        assets,
        maxAmountsIn,
...
async function createAndInitialisePool() {
    ...
    const weightedPoolFactoryAddr = '0x8e9aa87e45e92bad84d5f8dd1bff34fb92637de9';
    const assets = ['0x21ff756ca0cfcc5fff488ad67babadffee0c4149', '0xdFCeA9088c8A88A76FF74892C1457C17dfeef9C1'];
    const poolInfo = {
        name: 'WETH-STABAL3',
        symbol: 'WETH-STABAL3',
        tokens: assets,
    ...
    const address = await createPool(weightedPoolFactoryAddr, poolInfo, provider, wallet);
    await tokensApprove(poolInfo.tokens, balancerVault, MaxUint256.toString(), provider, wallet);
